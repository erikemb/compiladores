...
precedence = (
    ...
    ('left', 'VIRGULA'),
    ('right', 'DOIS_PONTOS'),
    ('right', 'DOIS_PONTOS_DUPLO'),
    ('left', 'AND'),
    ('left', 'OR'),
    ('left', 'NOT'),
    ('left', 'MAIOR', 'MENOR', 'MAIOR_IGUAL', 'MENOR_IGUAL', 'IGUAL', 'DIFERENTE'),
    ('left', 'PLUS', 'MINUS'),
    ('left', 'MULT', 'BARRA'),
    ('right', 'UMINUS'),  
    ('right', 'CIRCUNFLEXO'),
...
def p_topdecls(p):
    ...
    if len(p) == 4:  # topdecl PONTO_VIRGULA topdecls
        p[0] = [p[1]] + p[3]  # Concatena a declaração atual com a lista de declarações
    else:  # Apenas uma topdecl
        p[0] = [p[1]]  # Cria uma lista com uma única declaração

def p_topdecl(p):
    '''
    topdecl : decl
    '''
    p[0] = [p[1]] # p[1] é a decl
...
def p_lpat(p):
    ...
    if len(p) == 2:  # apat
        ...
        p[0] = ('UMINUS', p[2])  # Representa o unário
    else:  # gcon apatlist
        p[0] = (p[1], p[2])
...
def p_guard(p):
    ...
    if len(p) == 4:  # pat SETAS infixexp
        p[0] = (p[1], p[2], p[3])  # Armazena o padrão, SETAS e a expressão
    elif len(p) == 3:  # LET decls
        p[0] = (p[1], p[2])  # Armazena LET e decls
    else:  # infixexp
        p[0] = p[1]  # Armazena a expressão
...
def p_exptuple(p):
    ...
    if len(p) == 4:  # exp VIRGULA exptuple
        p[0] = [p[1]] + p[3]  # Concatena exp e exptuple
    else:  # exp VIRGULA exp
        p[0] = [p[1], p[3]]  # Armazena as duas expressões

def p_explist(p):
    '''explist : exp VIRGULA explist
               | exp'''
    if len(p) == 4:  # exp VIRGULA explist
        p[0] = [p[1]] + p[3]  # Concatena exp e explist
    else:  # exp
        p[0] = [p[1]]

def p_exp(p):
    '''exp : infixexp DOIS_PONTOS_DUPLO type
           | infixexp'''
    if len(p) == 4:  # infixexp DOISPONTOSDUPLO type
        p[0] = (p[1], p[2], p[3])  # Armazena a infixexp e o tipo
    else:  # infixexp
        p[0] = p[1]  # Armazena apenas a infixexp
...
def p_infixexp(p):
    ...
    if len(p) == 2:  # expop
        ...
        p[0] = ('UMINUS', p[2])  # Representa a operação unária
    else:  # lexp
        p[0] = p[1]
...
def p_expop(p):
    ...
    p[0] = (p[1], p[2], p[3])  # Armazena a operação
...
def p_lexp(p):
    ...
    if len(p) == 5:  # BARRA_INVERTIDA apatlist SETAS exp
        p[0] = (p[1], p[2], p[3], p[4])  # Armazena todos os componentes
    elif len(p) == 6:  # LET decls IN exp
        p[0] = (p[1], p[2], p[3])  # Armazena LET, decls e exp
    elif len(p) == 7:  # IF exp THEN exp ELSE exp
        p[0] = (p[1], p[2], p[3], p[4])  # Armazena IF, exp, THEN e ELSE
    elif len(p) == 6:  # CASE exp OF LCHAVE alts RCHAVE
        p[0] = (p[1], p[2], p[3], p[4])  # Armazena CASE, exp e alts
    else:  # fexp
        p[0] = p[1]  # Armazena o fexp

def p_fexp(p):
    '''fexp : aexp aexp
            | aexp fexp'''
    if len(p) == 3:  # aexp aexp
        p[0] = (p[1], p[2])  # Armazena aexp e aexp
    else:  # aexp fexp
        p[0] = (p[1], p[2])  # Armazena aexp e fexp
...
def p_quallist(p):
    ...
    if len(p) == 4:  # qual VIRGULA quallist
        p[0] = [p[1]] + p[3]  # Concatena qual e quallist
    else:  # qual
        p[0] = [p[1]]
...
def p_qual(p):
    ...
    if len(p) == 4:  # pat SETAS exp
        p[0] = (p[1], p[2], p[3])  # Armazena pat, SETAS e exp
    elif len(p) == 3:  # LET decls
        p[0] = (p[1], p[2])  # Armazena LET e decls
    else:  # exp
        p[0] = p[1]  # Armazena a expressão
...
def p_alts(p):
    ...
    if len(p) == 4:  # alt PONTO_VIRGULA alts
        p[0] = [p[1]] + p[3]  # Concatena alt e alts
    else:  # alt
        p[0] = [p[1]]
...
def p_alt(p):
    ...
    if len(p) == 5:  # pat SETAS exp optionwhere
        ...
        p[0] = (p[1], p[2], p[3])  # Armazena pat, gdpat e optionwhere

def p_optionwhere(p):
    '''optionwhere : WHERE decls
                   | '''
    if len(p) == 3:  # WHERE decls
        p[0] = p[2]  # Armazena decls
    else:  # vazio
        p[0] = None  # Representa a ausência de onde
...
def p_gdpat(p):
    ...
    if len(p) == 4:  # guards SETAS exp
        p[0] = (p[1], p[2], p[3])  # Armazena todos os componentes
...
def p_gcon(p):
    ...
    if len(p) == 3:  # LPAREN RPAREN ou LCOLCHETE RCOLCHETE
        p[0] = p[1]  # Armazena o tipo vazio
    elif len(p) == 4:  # LPAREN virgulalist RPAREN
        p[0] = (p[1], p[2], p[3])  # Armazena os elementos
    else:  # qcon
        p[0] = p[1]  # Armazena qcon

def p_virgulalist(p):
    '''virgulalist : VIRGULA virgulalist
                   | VIRGULA'''
    if len(p) == 3:  # VIRGULA virgulalist
        p[0] = [p[1]] + p[2]  # Concatena a lista
    else:  # apenas VIRGULA
        p[0] = [p[1]]  # Armazena uma única vírgula

def p_var(p):
    '''var : ID
           | LPAREN VARSYM RPAREN'''
    if len(p) == 2:  # ID
        p[0] = p[1]  # Armazena o identificador
    else:  # LPAREN VARSYM RPAREN
        p[0] = p[2]  # Armazena o símbolo

def p_qcon(p):
    '''qcon : LPAREN DOIS_PONTOS RPAREN'''
    p[0] = (p[1], p[2], p[3])  # Armazena os componentes

def p_varop(p):
    '''varop : VARSYM
             | CRASE ID CRASE'''
    if len(p) == 2:  # VARSYM
        p[0] = p[1]  # Armazena o símbolo
    else:  # CRASE ID CRASE
        p[0] = p[2]  # Armazena o identificador
...
def p_qop(p):
    '''qop : qvarop
           | DOIS_PONTOS'''
    p[0] = p[1]  # Armazena o qvarop ou os dois pontos
...
ast = parser.parse(codigo_teste, debug=True)
